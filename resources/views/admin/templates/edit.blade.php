...
<title>Edit Template CV - Admin</title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --panel: #ffffff;       
    --canvas-bg: #e5e5e5;   
    --item-card-bg: #f8fafc; /* Tambahan untuk List Item Card */
  ...
  body.dark-mode {
    ...
    --panel: #1f2937;
    --canvas-bg: #030712;
    --item-card-bg: #111827; /* Perbaikan warna list card saat dark mode */
...
<div class="topbar">
  <div class="topbar-left">
    ...
    <span class="topbar-logo">Resumate <span style="font-weight:400; color:var(--muted); font-size:13px; margin-left:5px;">(Edit Mode)</span></span>
  ...
  <div class="topbar-right">
    <a href="{{ route('admin.templates.index') }}" class="btn-top btn-ghost" style="text-decoration:none;"><i class="fas fa-times"></i> Batal</a>
    <button class="btn-top btn-ghost" onclick="promptResetCanvas()"><i class="fas fa-history"></i> Reset Data</button>
    <button class="btn-top btn-primary" onclick="openSaveModal()"><i class="fas fa-save"></i> Perbarui Template</button>
  </div>
</div>
...
<div id="saveModal" class="modal-overlay" style="display: none; z-index: 9999;">
  <div class="modal-content" style="width: 480px;">
    <h2 style="margin-bottom: 5px; font-size: 20px; color: var(--ink);">Perbarui Template</h2>
    <p style="font-size: 13px; color: var(--muted); margin-bottom: 24px;">Edit metadata template berikut ini.</p>
    
    <form id="laravelSaveForm" action="{{ route('admin.templates.update', $template->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        ...
        <div class="form-row">
          ...
          <input type="text" name="name" id="metaName" class="form-input" value="{{ old('name', $template->name) }}" required>
        </div>
        ...
        <div class="form-row">
          ...
          <div style="display:flex; align-items:center; gap:15px; margin-bottom: 10px;">
              <div style="border: 1px solid var(--line); border-radius: 4px; padding: 5px; width: 100px; min-height: 120px; background: var(--canvas-bg); display: flex; align-items: center; justify-content: center; overflow: hidden;">
                  <img id="thumbnailPreview" src="{{ asset('storage/' . $template->thumbnail) }}" alt="Preview" style="width: 100%; height: auto; display: block;">
                  <span id="thumbnailPlaceholder" style="font-size: 10px; color: var(--muted); text-align: center; display: none;">Belum ada gambar</span>
              </div>
              <button type="button" class="btn-top btn-ghost" onclick="captureThumbnail()" id="btnCapture" style="height: 38px;">
                  <i class="fas fa-camera"></i> Auto ScreenShoot CV
              </button>
          </div>
          <input type="file" name="thumbnail" id="metaThumbnail" class="form-input" accept="image/*" onchange="previewManualUpload(this)">
          <small style="color: var(--muted); font-size: 11px; display:block; margin-top:4px;">Bisa diisi manual atau klik "Auto Jepret CV". Biarkan kosong jika tidak ingin mengubah thumbnail.</small>
        </div>
        ...
        <div class="form-row-2">
          <div>
            ...
            <select name="category" id="metaCategory" class="form-select" required>
              <option value="Professional" {{ $template->category == 'Professional' ? 'selected' : '' }}>Professional</option>
              <option value="Creative" {{ $template->category == 'Creative' ? 'selected' : '' }}>Creative</option>
              <option value="Simple" {{ $template->category == 'Simple' ? 'selected' : '' }}>Simple</option>
              <option value="Akademik" {{ $template->category == 'Akademik' ? 'selected' : '' }}>Akademik</option>
            </select>
          </div>
          <div>
            <label class="form-label">Tipe Akses</label>
            <select name="type" id="metaType" class="form-select" onchange="togglePrice()" required>
              <option value="free" {{ $template->type == 'free' ? 'selected' : '' }}>Free (Gratis)</option>
              <option value="pro" {{ $template->type == 'pro' ? 'selected' : '' }}>Pro (Berlangganan)</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-row">
          ...
          <textarea name="description" id="metaDescription" class="form-textarea" style="min-height:50px;">{{ old('description', $template->description) }}</textarea>
        </div>

        <div class="form-row">
          <label class="form-label">Tags (Pencarian)</label>
          @php
             $tagsString = is_array($template->tags) ? implode(', ', $template->tags) : $template->tags;
          @endphp
          <input type="text" name="tags" id="metaTags" class="form-input" value="{{ old('tags', $tagsString) }}" placeholder="minimalis, bumn, mahasiswa">
        </div>
        ...
        <div class="form-row" style="margin-top:15px; background: var(--bg); padding: 12px; border-radius: 6px; border: 1px solid var(--line);">
          <label style="font-size:13px; cursor:pointer; display:flex; align-items:center; gap:8px;">
              <input type="checkbox" name="is_active" value="1" {{ $template->is_active ? 'checked' : '' }}> Status Publish (Aktif)
          </label>
        </div>

        <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 24px;">
          <button type="button" class="btn-top btn-ghost" onclick="closeSaveModal()">Batal</button>
          <button type="button" class="btn-top btn-primary" onclick="submitToLaravel()"><i class="fas fa-paper-plane"></i> Perbarui Template</button>
        </div>
    </form>
  </div>
</div>
...
<script>
// ============================================================
//  INTEGRASI DATA LARAVEL KE JAVASCRIPT (ANTI-ERROR)
// ============================================================
let rawSchema = {!! json_encode($template->layout_schema) !!};
let rawGlobal = {!! json_encode($template->global_settings) !!};

let initialSchema = [];
let initialGlobalSettings = { fontFamily: "'DM Sans', sans-serif" };

// Parse secara aman meskipun format dari database bervariasi
if (typeof rawSchema === 'string') {
    try { initialSchema = JSON.parse(rawSchema); } catch(e) {}
} else {
    initialSchema = rawSchema || [];
}
if (!Array.isArray(initialSchema)) {
    initialSchema = typeof initialSchema === 'object' && initialSchema !== null ? Object.values(initialSchema) : [];
}

if (typeof rawGlobal === 'string') {
    try { initialGlobalSettings = JSON.parse(rawGlobal); } catch(e) {}
} else {
    initialGlobalSettings = rawGlobal || { fontFamily: "'DM Sans', sans-serif" };
}
...
function captureThumbnail() {
    ...
    // Sembunyikan state selected agar outline tidak ikut ter-screenshot
...
function togglePrice(isInit = false) {
  ...
  if(type === 'free') { 
      ...
      if(!isInit && priceInput.value == 0) priceInput.value = 25000; 
  }
}
...
function promptResetCanvas() {
    customConfirm(
        'Reset Perubahan?', 
        'Semua perubahan yang belum disimpan akan dihapus. Kembali ke data asal?', 
        'fas fa-history', 
        'Ya, Reset', 
        'btn-primary', 
        function() {
            schema = JSON.parse(JSON.stringify(initialSchema));
            const font = initialGlobalSettings.fontFamily || "'DM Sans', sans-serif";
            document.getElementById('globalFont').value = font;
...
function updateIdCounter(arr) {
    arr.forEach(block => {
        const parts = block.id.split('_');
        if(parts.length >= 2) {
            const idNum = parseInt(parts[1]);
            if(!isNaN(idNum) && idNum > idCounter) idCounter = idNum;
        }
        if(block.isLayout) {
            if(block.layoutType === 'col1') updateIdCounter(block.children || []);
            else (block.columns || []).forEach(col => updateIdCounter(col));
        }
    });
}
...
function reattachDropZones(container, arr, parentId=null, colIndex=null) {
  ...
  container.addEventListener('drop', e => { onDrop(e, container, arr, parentId, colIndex); });
}
...
function deleteBlock(id, e) { 
    ...
    customConfirm(
        'Hapus Elemen?', 
        'Apakah Anda yakin ingin menghapus elemen ini dari design?', 
        'fas fa-trash-alt', 
        'Ya, Hapus', 
        'btn-danger', 
        function() {
...
function buildItemFields(type, blockId, idx, it) {
  ...
  switch(type) {
    ...
    // Perbaikan Bug Mapping Property Project di sini
...
function promptClearCanvas() { 
  ...
  customConfirm(
    'Konfirmasi Hapus', 
    'Apakah Anda yakin ingin menghapus semua elemen dari canvas?', 
    'fas fa-trash-alt', 
    'Ya, Hapus Semua', 
    'btn-danger', 
    function() {
...
// ============================================================
//  INIT APLIKASI SAAT HALAMAN DIMUAT (MODE EDIT)
// ============================================================
window.onload = function() {
  // Masukkan data dari Laravel ke variabel kerja (schema)
  if (typeof initialSchema !== 'undefined' && Array.isArray(initialSchema)) {
      schema = JSON.parse(JSON.stringify(initialSchema));
  }
  
  if (typeof initialGlobalSettings !== 'undefined' && initialGlobalSettings.fontFamily) {
      const font = initialGlobalSettings.fontFamily;
      document.getElementById('globalFont').value = font;
      document.getElementById('cv-paper').style.fontFamily = font;
  }
  
  updateIdCounter(schema);
  renderAll();
  saveState();
  togglePrice(true);
}
